      ...
      <div className=" mx-auto">
        <div className="mb-8">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <p className="text-gray-600 mt-1">Overview of all registered LCs within your organization.</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="block sm:hidden">
            {lcData.map((lc) => (
              <div key={lc.id} className="border-b border-gray-200 p-4 hover:bg-gray-50 transition-colors">
                <div className="flex justify-between items-start mb-3">
                  <div className="font-medium text-gray-900">{lc.lcNumber}</div>
                  <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${getStatusColor(lc.status)}`}>
                    {lc.status}
                  </span>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 text-right">{lc.beneficiary}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 text-right max-w-40 truncate" title={lc.products}>{lc.products}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-gray-900">{lc.totalAmount}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left py-4 px-6 font-semibold text-gray-900">LC Number</th>
                  <th className="text-left py-4 px-6 font-semibold text-gray-900">Beneficiary</th>
                  <th className="text-left py-4 px-6 font-semibold text-gray-900">Status</th>
                  <th className="text-left py-4 px-6 font-semibold text-gray-900">Open Date</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {lcData.map((lc) => (
                  <tr key={lc.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex px-3 py-1 text-sm font-medium rounded-full ${getStatusColor(lc.status)}`}>
                        {lc.status}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="text-gray-900 max-w-xs truncate" title={lc.products}>
                        {lc.products}
                      </div>
                    </td>
                    <td className="py-4 px-6 text-gray-900">{lc.quantity}</td>
                    <td className="py-4 px-6">
                      <div className="font-semibold text-gray-900">{lc.totalAmount}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
            <h3 className="text-xs sm:text-sm font-medium text-gray-500 mb-2">Total LCs</h3>
            <p className="text-2xl sm:text-3xl font-bold text-gray-900">{lcData.length}</p>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 col-span-2 lg:col-span-1">
            <h3 className="text-xs sm:text-sm font-medium text-gray-500 mb-2">Total Value</h3>
            <p className="text-2xl sm:text-3xl font-bold text-blue-600">
              ${lcData.reduce((sum, lc) => sum + parseFloat(lc.totalAmount.replace(/[$,]/g, '')), 0).toLocaleString()}
            </p>
          </div>
        </div>
      </div>
