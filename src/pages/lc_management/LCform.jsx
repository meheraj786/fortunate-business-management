...
const LCForm = ({ onSave, editData = null }) => {
  ...
  useEffect(() => {
    if (editData) {
      const productsWithIds = editData.product_info.map(p => p.id ? p : {...p, id: productIdCounter++});
      setFormData({...editData, product_info: productsWithIds});
    }
  }, [editData]);
  ...
  const handleProductSpecChange = (id, field, value) => {
    const updatedProducts = formData.product_info.map(p => 
      p.id === id ? { ...p, specification: { ...p.specification, [field]: value } } : p
    );
    setFormData(prev => ({ ...prev, product_info: updatedProducts }));
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button type="submit" className="px-6 py-2 bg-[#003b75] text-white rounded-lg hover:bg-[#002a54] transition-colors duration-200 font-medium">
              Save LC
            </button>
          </div>
        </form>
