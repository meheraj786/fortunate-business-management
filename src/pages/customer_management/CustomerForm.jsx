...
const CustomerForm = ({ onClose, onSave, editData = null }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.basicInfo.fullName || !formData.contactInfo.phoneNumber || !formData.contactInfo.email) {
            ...
            >
              {section.id === 'basic' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <InputField label="Full Name" value={formData.basicInfo.fullName} onChange={(v) => handleInputChange('basicInfo', 'fullName', v)} required placeholder="Ahmed Hassan" icon={User} />
                  <InputField label="Company Name" value={formData.basicInfo.companyName} onChange={(v) => handleInputChange('basicInfo', 'companyName', v)} placeholder="Hassan Trading" icon={Building} />
                  <SelectField label="Customer Type" value={formData.basicInfo.customerType} onChange={(v) => handleInputChange('basicInfo', 'customerType', v)} options={customerTypes} required icon={Building} />
                  <SelectField label="Customer Status" value={formData.basicInfo.customerStatus} onChange={(v) => handleInputChange('basicInfo', 'customerStatus', v)} options={statusOptions} required icon={Shield} />
                  <InputField label="Customer Join" type="date" value={formData.basicInfo.joinDate} onChange={(v) => handleInputChange('basicInfo', 'joinDate', v)} required icon={Calendar} />
                  <InputField label="Credit Limit" type="text" value={formData.basicInfo.creditLimit} onChange={(v) => handleInputChange('basicInfo', 'creditLimit', v)} placeholder="5000" icon={DollarSign} />
                </div>
              )}
              {section.id === 'contact' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <InputField label="Phone Number" value={formData.contactInfo.phoneNumber} onChange={(v) => handleInputChange('contactInfo', 'phoneNumber', v)} required placeholder="+880 1712-345678" icon={Phone} />
                  <InputField label="Alternative Phone Number" value={formData.contactInfo.alternativePhoneNumber} onChange={(v) => handleInputChange('contactInfo', 'alternativePhoneNumber', v)} placeholder="+880 1712-345678" icon={Phone} />
