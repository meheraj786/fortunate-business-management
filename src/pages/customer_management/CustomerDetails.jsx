...
import { FiUser, FiMail, FiPhone, FiMapPin, FiDollarSign, FiFileText, FiCalendar, FiCheckCircle, FiXCircle, FiAlertCircle, FiBriefcase, FiStar, FiDownload, FiPieChart } from 'react-icons/fi';
...
const CustomerDetails = () => {
  ...
  const totalSpentAmount = customerSales.reduce((acc, sale) => acc + sale.totalAmount, 0);
  const totalNotInvoiced = customerSales.filter((sale) => sale.invoiceStatus === 'Not Invoiced').length;
  const totalOutstandingDues = customerSales.reduce((acc, sale) => {
    if (sale.paymentStatus === 'Due Payment') {
      const paidAmount = sale.payments.reduce((acc, payment) => acc + payment.amount, 0);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{customerData.name}</h1>
              <div className="flex items-center mt-1">
                <span className="text-gray-600 mr-3">{customerData.customerId}</span>
          ...
          <div className="space-y-4 lg:order-1 lg:col-span-2">
            <CollapsibleCard title="General Info" icon={<FiUser />}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <DataField label="Name" value={customerData.name} icon={<FiUser />} />
                <DataField label="Company Name" value={customerData.companyName} icon={<FiBriefcase />} />
                <DataField label="Phone Number" value={customerData.phone} icon={<FiPhone />} />
                <DataField label="Email" value={customerData.email} icon={<FiMail />} />
                <DataField label="Billing Address" value={customerData.billingAddress} icon={<FiMapPin />} className="sm:col-span-2" />
                <DataField label="Credit Limit" value={`${customerData.creditLimit.toLocaleString()}`} icon={<FiDollarSign />} />
              </div>
            </CollapsibleCard>
            ...
            <div className="lg:hidden">
              <CollapsibleCard title="Status Info" icon={<FiStar />}>
                <div className="space-y-3">
                  ...
                  <DataField label="Customer Joined Date" value={customerData.joinDate} icon={<FiCalendar />} />
                </div>
              </CollapsibleCard>
            </div>
            ...
            <div className="lg:hidden">
              <CollapsibleCard title="Others" icon={<FiFileText />}>
                <DataField label="Customer Note" value={customerData.customerNote} icon={<FiFileText />} className="sm:col-span-2" />
            ...
            <CollapsibleCard title="Transaction Overview" icon={<FiPieChart />}>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg text-center">
                  <div className="text-2xl font-bold text-blue-700">{totalPurchasesCount}</div>
                  <div className="text-sm text-[#003b75]">Total Purchases</div>
                </div>
                <div className="bg-green-50 p-4 rounded-lg text-center">
                  <div className="text-2xl font-bold text-green-700">${totalSpentAmount.toLocaleString()}</div>
                  <div className="text-sm text-green-600">Total Spent</div>
                </div>
                <div className="bg-yellow-50 p-4 rounded-lg text-center">
                  <div className="text-2xl font-bold text-yellow-700">{totalNotInvoiced}</div>
                  <div className="text-sm text-yellow-600">Not Invoiced</div>
                </div>
                <div className="bg-red-50 p-4 rounded-lg text-center">
                  <div className="text-2xl font-bold text-red-700">${totalOutstandingDues.toLocaleString()}</div>
            ...
            <div className="block sm:hidden">
              <div className="space-y-2">
                {customerSales.map((sale) => (
                  <div key={sale.id} className="border-t border-gray-200 last:border-b bg-white cursor-pointer hover:bg-gray-50 transition-colors">
                ...
                {customerSales.length === 0 && (
                  <div className="text-center py-8 text-gray-500 border-t">No sales data available for this customer.</div>
                )}
              </div>
            </div>
            <div className="hidden sm:block overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {customerSales.map((sale) => (
                    <tr key={sale.id}>
                      <td className="px-4 py-3 whitespace-nowrap text-gray-500">{sale.saleDate}</td>
                      <td className="px-4 py-3 whitespace-nowrap font-medium text-[#003b75]">{sale.productName}</td>
                      <td className="px-4 py-3 whitespace-nowrap text-gray-500">{sale.lcNumber || 'N/A'}</td>
                      <td className="px-4 py-3 whitespace-nowrap text-gray-500">{sale.quantity} {sale.unit}</td>
                      <td className="px-4 py-3 whitespace-nowrap text-gray-500">${sale.pricePerUnit.toLocaleString()}</td>
                      <td className="px-4 py-3 whitespace-nowrap text-gray-500">${sale.totalAmount.toLocaleString()}</td>
