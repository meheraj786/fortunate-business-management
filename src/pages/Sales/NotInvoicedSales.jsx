...
const NotInvoicedSales = () => {
  ...
  const notInvoicedSales = useMemo(() => {
    return initialSalesData.filter(sale => sale.invoiceStatus === 'Not Invoiced');
  }, []);

  const filteredSales = useMemo(() => {
    return notInvoicedSales.filter(sale =>
      sale.productName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      sale.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      sale.id.toString().includes(searchTerm)
    );
  }, [notInvoicedSales, searchTerm]);
      ...
      <div className="mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <p className="text-gray-600 text-sm sm:text-base">
            Sales that have not been invoiced yet.
          </p>
        </div>
