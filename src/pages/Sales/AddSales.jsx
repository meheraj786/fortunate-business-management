...
const AddSales = ({
  ...
  const availableProducts = useMemo(() => {
    ...
    let filtered = products.filter(
      (p) => p.warehouse?._id === formData.warehouseId
    );
  ...
  useEffect(() => {
    if (isEditMode) return; // Prevent this logic from running in edit mode
  ...
  useEffect(() => {
    // This hook should not run for sales that were already invoiced when loaded.
    // It should run for new sales or sales being updated from not-invoiced to invoiced.
