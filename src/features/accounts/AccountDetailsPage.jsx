...
const AccountDetails = () => {
  ...
  const fetchAccountDetails = useCallback(async () => {
    ...
    try {
      const accountRes = await api.get(
        `/account/get-account-details/${accountId}`
      );
      console.log("API Response:", accountRes.data);
      if (accountRes.data.success) {
        setAccount(accountRes.data.data.account);
        setAccountStats(accountRes.data.data.stats);
        console.log("Account and Stats set:", accountRes.data.data);
      } else {
    ...
    async (page = 1) => {
      ...
      try {
        ...
        const transRes = await api.get(
          `/transactions/get-transactions-by-account/${accountId}?${params.toString()}`
        );
  ...
  return (
    <div>
      ...
      {accountStats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          ...
          <StatBox
            ...
            number={`৳${accountStats.averageTransactionAmount.toLocaleString()}`}
            Icon={DollarSign}
            textColor="default"
          />
        </div>
      ...
      {isBank && (
        <AddBankAccountForm
          isOpen={editingAccountType === "Bank"}
          onClose={() => setEditingAccountType(null)}
      ...
      {isMobile && (
        <AddMobileAccountForm
          isOpen={editingAccountType === "Mobile Banking"}
          onClose={() => setEditingAccountType(null)}
          editingAccount={account}
          onSuccess={handleFormSuccess}
        />
      ...
      {isCash && (
        <AddCashAccountForm
          isOpen={editingAccountType === "Cash"}
          onClose={() => setEditingAccountType(null)}
          editingAccount={account}
          onSuccess={handleFormSuccess}
        />
